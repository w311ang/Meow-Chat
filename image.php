...
else
{
    ...
    else
    {
		...
		if(file_exists(storage.''.chatfile)) {
			...
			fwrite($myfile,$ip . "+" . $timehidden . "" . linkchar1 . "[".$time."] [" . $un . "] " .linkchar2 . "" .  "<img src=\"".str_replace(dirname(__FILE__), '', ".".$filename)."\""." width=\"80px\""." />" . "\r\n");
			fclose($myfile);
		} else {
			file_put_contents(storage.''.chatfile, $ip . "+" . $timehidden . "" . linkchar1 . "[".$time."] [" . $un . "] " .linkchar2 . "" . "<img src=\"".str_replace(dirname(__FILE__), '', ".".$filename)."\""." width=\"80px\""." />" . "\r\n");
		};
		$etime=microtime(true);
		$totaltime=$etime-$stime;
